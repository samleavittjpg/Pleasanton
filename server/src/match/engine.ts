...
import type { HouseIndex, MatchLengthMinutes, MatchState, NeighborhoodState, PlayerId } from "./types.js";
...
export function createMatch(store: MatchStore, params: CreateMatchParams, creator: Player): MatchState {
  const id = randomUUID();
...
export function joinMatch(store: MatchStore, matchId: string, player: Player): MatchState | undefined {
  const match = store.matches.get(matchId);
  if (!match) return undefined;
  if (match.status !== "lobby") return match;
  if (match.players.some((p) => p.id === player.id)) return match;
  if (match.players.length >= MVP_MAX_PLAYERS) return match;
...
function maybeStartMatch(match: MatchState) {
  if (match.status !== "lobby") return;
  if (match.players.length < MVP_MIN_PLAYERS) return;
  match.status = "running";
  match.startedAtMs = Date.now();
  match.tick = 0;
}

function newNeighborhood(ownerPlayerId: PlayerId): NeighborhoodState {
  const houses = Array.from({ length: 10 }, (_, i) => ({
    index: i as HouseIndex,
    graffiti01: 0,
    trashed01: 0,
    foreclosed: false,
    graffitiDataUrl: undefined as string | undefined,
  }));
...
function computeMetrics(houses: NeighborhoodState["houses"]) {
...
export function stepMatch(match: MatchState): MatchState {
  if (match.status !== "running" || !match.startedAtMs) return match;
...
function bestPlayer(match: MatchState): { playerId: PlayerId; avgHappiness: number } | undefined {
  let best: { playerId: PlayerId; avgHappiness: number } | undefined;
  for (const p of match.players) {
    const n = match.neighborhoods[p.id];
    if (!n) continue;
    if (!best || n.avgHappiness > best.avgHappiness) best = { playerId: p.id, avgHappiness: n.avgHappiness };
  }
  return best;
}

function finishMatch(match: MatchState, winnerPlayerId?: PlayerId) {
  match.status = "finished";
  match.endedAtMs = Date.now();
  if (winnerPlayerId) match.winnerPlayerId = winnerPlayerId;
}
