export type HappinessComponents = {
  cleanliness: number; // 0..1 (1 = spotless)
  graffiti: number; // 0..1 (1 = no graffiti)
  foreclosures: number; // 0..1 (1 = none foreclosed)
  schoolFunding: number; // 0..1
  sanitationFunding: number; // 0..1
  parksFunding: number; // 0..1
  policingFunding: number; // 0..1
  heatPenalty: number; // 0..1 (1 = no penalty)
};
 ...
 */
export function computeAverageHappiness(components: HappinessComponents): HappinessBreakdown {
  const c: HappinessComponents = {
  ...
  const weights: HappinessBreakdown["weights"] = {
    cleanliness: 2.0,
    graffiti: 1.6,
    foreclosures: 2.4,
    schoolFunding: 1.2,
  ...
  const weighted =
    c.cleanliness * weights.cleanliness +
