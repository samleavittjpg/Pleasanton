...
export function applyAction(params) {
    ...
    switch (action.kind) {
        case "maintenance/cleanStreet": {
            ...
            actor.budget -= cost;
            for (const h of actor.houses)
                h.trashed01 = clamp01(h.trashed01 - 0.12);
            return { ok: true };
        }
        case "maintenance/removeGraffiti": {
            ...
            if (house.graffiti01 < 0.05)
                house.graffitiDataUrl = undefined;
            return { ok: true };
        }
        ...
        case "sabotage/dumpTrash": {
            ...
            if (target.ownerPlayerId === actor.ownerPlayerId)
                return { ok: false, message: "Nice try." };
            const cost = 6;
            if (actor.budget < cost)
                return { ok: false, message: "Not enough budget." };
            actor.budget -= cost;
            ...
            for (const h of target.houses)
                h.trashed01 = clamp01(h.trashed01 + 0.08);
            return { ok: true };
        }
        case "sabotage/graffitiHouse": {
            ...
            if (!house)
                return { ok: false, message: "Invalid house." };
            if (house.foreclosed)
                return { ok: false, message: "House is foreclosed." };
            const cost = 7;
            if (actor.budget < cost)
                return { ok: false, message: "Not enough budget." };
            actor.budget -= cost;
